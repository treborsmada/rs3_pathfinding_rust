...
impl MapSection {
    pub fn bd_range(&self, x: u16, y: u16) -> Vec<(u16, u16, u8)> {
        ...
        let mut tiles = Vec::with_capacity(441);
        for i in 0..7 {
    ...
    pub fn create_map_section(x_start: usize, x_end: usize, y_start: usize, y_end: usize, floor: usize) -> MapSection {
        let move_data = build_move_array(x_start, x_end, y_start, y_end, floor);
    ...
    pub fn walk_range_2(&self, x: u16, y: u16) -> &Vec<(u16, u16, u8)> {
        self.walk_hashmap.get(&(x, y)).unwrap()
    }

    pub fn bd_range_2(&self, x: u16, y: u16) -> &Vec<(u16, u16, u8)> {
        self.bd_hashmap.get(&(x, y)).unwrap()
    }
}

fn build_surge_hashmap(x_start: usize, x_end: usize, y_start: usize, y_end: usize, arr: &Array3<u8>) -> HashMap<(u16, u16, u8), (u16, u16)> {
    let mut surge_hashmap = HashMap::new();
    for x in x_start..=x_end {
        for y in y_start..=y_end {
            for direction in 0..8 {
                let offset = (arr[[x - x_start, y - y_start, direction as usize]] & 15) as u16;
                let x = x as u16;
                let y= y as u16;
                match direction {
                    0 => {surge_hashmap.insert((x, y, direction), (x, y + offset));},
                    1 => {surge_hashmap.insert((x, y, direction),(x + offset, y + offset));},
                    2 => {surge_hashmap.insert((x, y, direction),(x + offset, y));},
                    3 => {surge_hashmap.insert((x, y, direction),(x + offset, y - offset));},
                    4 => {surge_hashmap.insert((x, y, direction),(x, y - offset));},
                    5 => {surge_hashmap.insert((x, y, direction),(x - offset, y - offset));},
                    6 => {surge_hashmap.insert((x, y, direction),(x - offset, y));},
                    7 => {surge_hashmap.insert((x, y, direction),(x - offset, y + offset));},
                    _ => panic!()
                }
            }
        }
...
fn build_move_array(x_start: usize, x_end: usize, y_start: usize, y_end: usize, floor: usize) -> Array2<u8> {
    let chunk_size =  1280;
    ...
    for j in chunk_y.0..=chunk_y.1 {
        ...
        for i in chunk_x.0..=chunk_x.1 {
            let path = format!("MapData/Map/move-{i}-{j}-{floor}.npy");
            let arr: Array2<u8> = read_npy(path).unwrap();
